...
int ca_bicgstab(CSR_Matrix *A_loc, INFO_Matrix *A_info, double *x, double *r) {
    ...
    rTw = my_ddot(vec_loc_size, r_loc, w_loc);
    MPI_Iallreduce(MPI_IN_PLACE, &rTw, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD, &rTw_req);
    MPI_Wait(&rTr_req, MPI_STATUS_IGNORE);
    ...
    while (dot_r > tol * tol * dot_zero && k < max_iter) {
        ...
        rTz = my_ddot(vec_loc_size, r_hat_loc, z_loc);
        MPI_Iallreduce(MPI_IN_PLACE, &rTz, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD, &rTz_req);
        my_daxpy(vec_loc_size, -alpha, z_loc, w_loc);
        rTw = my_ddot(vec_loc_size, r_loc, w_loc);
        MPI_Iallreduce(MPI_IN_PLACE, &rTw, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD, &rTw_req);
        wTw = my_ddot(vec_loc_size, w_loc, w_loc);
        MPI_Iallreduce(MPI_IN_PLACE, &wTw, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD, &wTw_req);
        MPI_Wait(&rTw_req, MPI_STATUS_IGNORE);
        ...
        csr_mv(A_loc, r, w_loc);
        rTw = my_ddot(vec_loc_size, r_hat_loc, w_loc);
        MPI_Iallreduce(MPI_IN_PLACE, &rTw, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD, &rTw_req);
        MPI_Wait(&rTs_req, MPI_STATUS_IGNORE);
        ...
        MPI_Wait(&dot_r_req, MPI_STATUS_IGNORE); //ほとんど影響なし
        if (myid == 0 && k % 100 == 0) {
            printf("Iteration: %d, Residual: %e\n", k, sqrt(dot_r / dot_zero));
        }
    }
